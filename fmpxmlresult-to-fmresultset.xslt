...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:FM="http://www.filemaker.com/fmpxmlresult" exclude-result-prefixes="FM">
  ...
  <xsl:key name="field-by-table" match="FM:FIELD" use="substring-before(@NAME, '::')" />
  <!-- Fields, grouped by table (i.e. first field from each table) -->
  <xsl:variable name="first-table-field" select="/FM:FMPXMLRESULT /FM:METADATA /FM:FIELD[ count( . | key( 'field-by-table', substring-before(@NAME, '::') )[1]) = 1 ]" />
  <xsl:template match="/">
    <xsl:for-each select="FM:FMPXMLRESULT">
      <fmresultset xmlns="http://www.filemaker.com/xml/fmresultset" version="1.0">
        ...
        <xsl:for-each select="FM:DATABASE">
          <datasource database="{@NAME}" table="N/A" layout="{@LAYOUT}" total-count="{@RECORDS}" date-format="{@DATEFORMAT}" time-format="{@TIMEFORMAT}" timestamp-format="{@DATEFORMAT} {@TIMEFORMAT}" />
        </xsl:for-each>
        <xsl:for-each select="FM:METADATA">
          <metadata>
            ...
            <xsl:for-each select="$first-table-field">
              ...
              <xsl:variable name="table-fields" select="key( 'field-by-table', $table )" />
              <xsl:choose>
                ...
                <xsl:for-each select="$first-table-field">
                  ...
                  <xsl:choose>
                    <xsl:when test="not(string($table))">
                      ...
                      <xsl:for-each select="$table-fields">
                        <xsl:variable name="col" select="count( preceding-sibling::FM:FIELD ) + 1" />
                        <field name="{@NAME}">
                    ...
                    <xsl:otherwise>
                      ...
                      <xsl:variable name="my-col" select="count( preceding-sibling::FM:FIELD ) + 1" />
                      <xsl:variable name="my-col-data" select="$row/FM:COL[$my-col]/FM:DATA" />
                      <relatedset table="{$table}" count="{count($my-col-data)}">
                        <xsl:for-each select="$my-col-data">
                          ...
                          <record record-id="{position()}">
                            <xsl:for-each select="$table-fields">
                              <xsl:variable name="col" select="count( preceding-sibling::FM:FIELD ) + 1" />
                              <field name="{@NAME}">
                                <data><xsl:value-of select="$row /FM:COL[$col]/FM:DATA[$my-row]" /></data>
                              </field>
                            </xsl:for-each>
                          </record>
                        </xsl:for-each>
  ...
  <xsl:template name="field-definition">
    <field-definition xmlns="http://www.filemaker.com/xml/fmresultset" name="{@NAME}" max-repeat="{@MAXREPEAT}" result="{translate(@TYPE, $UC, $lc)}" type="normal" auto-enter="no" four-digit-year="no" time-of-day="no" numeric-only="no" global="no">
